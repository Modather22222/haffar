...
class _SparkleBurstOverlayState extends State<SparkleBurstOverlay> with SingleTickerProviderStateMixin {
  late final AnimationController _controller;
  late final List<_Particle> _particles;
  Size _size = Size.zero;

  static const _colors = [HaffarColors.primary, Color(0xFFff9600), Color(0xFFffc800), HaffarColors.primaryLight, Color(0xFFa855f7)];
    ...
    return Positioned(
      ...
      child: Opacity(
        ...
        child: Container(
          width: r * 2,
          height: r * 2,
          decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: const Color(0xFFffc800), width: 3)),
        ),
      ),
    );
  }
  ...
  Widget _buildParticle(_Particle p, Offset origin) {
    ...
    final pos = origin + Offset(math.cos(p.angle), math.sin(p.angle)) * p.distance * travel;

    Widget shape = p.isDot
        ? Container(width: p.size * 0.5, height: p.size * 0.5, decoration: BoxDecoration(color: p.color, shape: BoxShape.circle))
        : Icon(Icons.auto_awesome, size: p.size, color: p.color);

    return Positioned(
      left: pos.dx,
      top: pos.dy,
      child: Opacity(opacity: opacity, child: Transform.rotate(angle: p.spin * travel, child: Transform.scale(scale: (popIn * shrink).clamp(0.0, 1.5), child: shape))),
    );
  }
}
...
class _Particle {
  ...
  const _Particle({required this.angle, required this.distance, required this.size, required this.color, required this.spin, required this.isDot});
}
