...
class _TrueFalseQuestionState extends State<TrueFalseQuestion> {
  ...
  Widget build(BuildContext context) {
    return QuestionBase(
      ...
      buildBody: (_) => Column(children: [
        ...
        TextButton(onPressed: widget.locked ? null : (_selected == null ? widget.onSkip : null), child: const Text('تخطي', style: TextStyle(fontFamily: 'BeVietnamPro', fontSize: 14, fontWeight: FontWeight.w500, color: HaffarColors.outline))),
      ]),
    );
  }
  ...
  Widget _tfButton(String label, IconData icon, bool value) {
    ...
    return Material(color: Colors.transparent, child: InkWell(onTap: widget.locked ? null : () => _select(value), borderRadius: BorderRadius.circular(16),
      child: AnimatedContainer(duration: const Duration(milliseconds: 150),
        decoration: BoxDecoration(color: bgColor, borderRadius: BorderRadius.circular(16), border: Border.all(color: borderColor),
          boxShadow: !isSelected ? [BoxShadow(color: HaffarColors.outline.withValues(alpha: 0.08), blurRadius: 4, offset: const Offset(0, 2))] : []),
        child: Padding(padding: const EdgeInsets.symmetric(vertical: 20), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(icon, color: textColor, size: 28),
          const SizedBox(width: 10),
          Text(label, style: TextStyle(fontFamily: 'BeVietnamPro', fontSize: 18, fontWeight: FontWeight.w700, color: textColor)),
        ])),
      ),
    ));
  }
