...
class _ClassificationQuestionState extends State<ClassificationQuestion> {
  ...
  void _check() {
    ...
    for (var i = 0; i < widget.items.length; i++) {
      ...
      if (_placements[i].toString() != expected) {
        correct = false;
        hint ??=
            '${widget.items[i]}: ${expected == '0' ? widget.zoneOneLabel : widget.zoneTwoLabel}';
      }
    }
        ...
        children: [
          ...
          Container(
            ...
            child: Wrap(
              ...
              children: [
                for (var i = 0; i < widget.items.length; i++)
                  if (!_placements.containsKey(i)) _chip(i, inBank: true),
              ],
            ),
          ),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              style: ElevatedButton.styleFrom(
                backgroundColor: _allPlaced && _interactive
                    ? HaffarColors.primary
                    : HaffarColors.surfaceHigh,
                foregroundColor: _allPlaced && _interactive
                    ? Colors.white
    ...
    return Column(
      children: [
        Container(
          ...
          child: Text(
            isGreen ? widget.zoneOneLabel : widget.zoneTwoLabel,
        ...
        InkWell(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(12),
              border: Border.all(
                color: _selectedItem != null && _interactive
                    ? accent.withValues(alpha: 0.7)
                    : accent.withValues(alpha: 0.25),
                width: _selectedItem != null && _interactive ? 2 : 1.5,
              ),
            ),
            ...
            child: _placements.entries.where((e) => e.value == zone).isEmpty
                ? Center(
                    child: Text(
                      _selectedItem != null && _interactive
                          ? 'اضغط للوضع هنا'
                          : '',
                      style: TextStyle(
                ...
                : Wrap(
                    ...
                    children: [
                      for (final e in _placements.entries)
                        if (e.value == zone) _chip(e.key, inBank: false),
                    ],
                  ),
          ),
        ),
  ...
  Widget _chip(int idx, {required bool inBank}) {
    ...
    return GestureDetector(
      onTap: inBank ? () => _tapBankChip(idx) : () => _tapPlacedChip(idx),
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 150),
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: border,
            width: selectedInBank || (!inBank && _submitted) ? 2 : 1.5,
          ),
          boxShadow: selectedInBank
              ? [
                  BoxShadow(
        ...
        child: Row(
          ...
          children: [
            if (!inBank && _submitted) ...[
              Icon(
                _placements[idx].toString() == widget.itemCategories[idx]
                    ? Icons.check_circle
                    : Icons.cancel,
                size: 15,
                color: bg == Colors.white ? _green : border,
              ),
