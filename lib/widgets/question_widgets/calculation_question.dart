...
class _CalculationQuestionState extends State<CalculationQuestion> {
  static const _arabicDigits = {'٠': '0', '١': '1', '٢': '2', '٣': '3', '٤': '4', '٥': '5', '٦': '6', '٧': '7', '٨': '8', '٩': '9'};
  ...
  bool get _canSubmit {
    ...
    return _isChoiceMode ? _selected != null : _normalize(_controller.text).isNotEmpty;
  }
  ...
  void _check() {
    ...
    if (_isChoiceMode) {
      ...
      setState(() { _submitted = true; _isCorrect = found; });
      ...
      buildBody: (_) => Column(children: [
        if (widget.formula != null && !_isChoiceMode) ...[
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFFc8e6ff).withValues(alpha: 0.35), borderRadius: BorderRadius.circular(12), border: Border.all(color: HaffarColors.primaryLight.withValues(alpha: 0.5))),
            child: Row(children: [
              const Icon(Icons.functions, size: 20, color: Color(0xFF006590)),
              const SizedBox(width: 10),
              Expanded(child: Text(widget.formula!, style: const TextStyle(fontFamily: 'PlusJakartaSans', fontSize: 15, fontWeight: FontWeight.w500, color: Color(0xFF006590)))),
            ]),
          ),
        ...
        if (_isChoiceMode)
          ...widget.options.asMap().entries.map((entry) {
            ...
            return Padding(padding: const EdgeInsets.only(bottom: 10), child: _CalcOption(
              text: text,
              isSelected: _selected == idx && !_submitted,
              isCorrect: widget.locked && !_isChoiceMode ? false : _submitted && idx == widget.correctIndex,
              isWrong: _submitted && _selected == idx && idx != widget.correctIndex,
              onTap: widget.locked || _submitted ? null : () => setState(() => _selected = idx),
            ));
          })
        else
          TextField(
            ...
            style: const TextStyle(fontFamily: 'BeVietnamPro', fontSize: 22, fontWeight: FontWeight.w700),
        ...
        Padding(padding: const EdgeInsets.symmetric(horizontal: 0), child: Column(children: [
          SizedBox(height: 52, width: double.infinity, child: ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: _canSubmit ? HaffarColors.primary : HaffarColors.surfaceHigh, foregroundColor: _canSubmit ? Colors.white : HaffarColors.textSecondary, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
...
class _CalcOption extends StatelessWidget {
  ...
  const _CalcOption({required this.text, required this.isSelected, required this.isCorrect, required this.isWrong, this.onTap});
